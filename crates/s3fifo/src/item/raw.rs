...
impl RawItem {
    ...
    pub(crate) fn wrapping_add(&mut self, rhs: u64) -> Result<(), S3FifoError> {
        match self.value() {
            Value::U64(v) => unsafe {
            ...
            _ => Err(S3FifoError::NotNumeric),
        }
    }

    pub(crate) fn saturating_sub(&mut self, rhs: u64) -> Result<(), S3FifoError> {
        match self.value() {
            Value::U64(v) => unsafe {
